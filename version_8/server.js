...
// OpenAI API 키와 엔드포인트
const apiKey = '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';
...
async function fetchAIResponse(prompt,number) {
    const a = `다음 텍스트에서 상품명과 냉동식품과 냉장식품을 구별하여 한 줄씩 정리해줘(예를 들어 아이스크림 1개가 텍스트에 있으면 냉동:아이스크림-1개):\n${prompt}`;
    //const b = `${prompt}들을 재료로 써서 만들 수 요리와 레시피들을 3개씩 정리해줘(예를 들어 1.요리이름 2.레시피 첫째줄 3. 레시피 둘째줄 1.다른요리이름 2.다른요리 레시피 첫쨰줄):`;
    const b = `${prompt}들을 재료로 써서 만들 수 요리이름 5개 정리해줘(요리이름 하나당 10글자 이상 20글자가 이하 정도로 해줘))`;
    if(number == 1){
